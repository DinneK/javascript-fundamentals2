...
describe("Recipe", function () {
  it("should have a name and ingredients", function () {
    ...
    var quesadillas = new Recipe({
      name: "quesadillas",
      ingredients: [cheese, tortillas],
    });
  ...
  it("should start as unattempted with no rating", function () {
    var chocolateChips = new Ingredient({
    ...
    var cookies = new Recipe({
      ...
      ingredients: [chocolateChips, flour, sugar, bakingSoda],
    });
  ...
  it("should be able to generate a grocery list", function () {
    ...
    assert.equal(
      groceryList,
      "You need: 1 rotisserie chicken, 3 carrots, 2 celery, 1 yellow onion, 4 chicken broth, 1 egg noodles,"
    );
  });
});
